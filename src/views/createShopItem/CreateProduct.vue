<template>
  <div class="tab_title"><span>创建商品（5855）</span></div>
  ...
  />
    ...
    <el-tab-pane label="规格/库存" name="second">
      ...
      <el-form-item label="SkU列表" v-show="ruleForm2.standard_type==='多规格'">
        {{ JSON.stringify(skuTablelist) }}
        {{ multiSpec }}
     <el-table :data="skuTablelist" width="500px">
...
<template #default="scope">
  <el-upload
    ...
    :multiple="true"
    >
    <img v-if="scope.row.pic" :src="scope.row.pic" class="avatar" style="width:100px;height:100px">
    <el-icon v-else class="avatar-uploader-icon"><Plus /></el-icon>
  </el-upload>
</template>
  </el-table-column>
...
<template #default="scope">
  <el-input type="text" style="width:88px" @blur="skuSetprice($event,scope.$index)" v-model="price2"/>
</template>
  </el-table-column>
...
let price2=ref('')
  ...
  let good_name=ruleForm.value.name
  const ruleForm2 = ref({
    standard_type:'单规格',
    shop_price: 1.00,
    line_price:0.00,
...
function typeList(){
    ...
    function skuSetprice(e,index){
      console.log(e)
      console.log(price2.value)
      skuTablelist.value[index].price=price2.value
    }
